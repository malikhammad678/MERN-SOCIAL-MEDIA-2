...
import { dummyConnectionsData as connections, dummyFollowingData as followings, dummyFollowersData as followers, dummyPendingConnectionsData as pendingConnections } from "../assets/assets"
...
const Connections = () => {
  ...
  const dataArray = [
    ...
    { label:'Following', value:followings, icon:UserCheck },
        ...
        {/* Title */}
         ...
         <div className='mb-8'>
          ...
          <div className="inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm">
             {
              dataArray.map((tab) => (
                <button onClick={() => setCurrentTab(tab.label)} key={tab.label} className={`flex items-center px-3 py-1 text-sm rounded-md cursor-pointer transition-colors ${currentTab === tab.label ? 'bg-white font-medium text-black' : 'text-gray-500 hover:text-black'}`}>
                    ...
                    <div className="flex max-sm:flex-col gap-2 mt-4">
                      ...
                      }
                      {
                        currentTab === 'Connections' && (
                          <button onClick={() => navigate(`/messages/${user._id}`)} className="w-full p-2 text-sm rounded bg-slate-100 hover:bg-slate-200 text-slate-800 flex items-center justify-center gap-1 active:scale-95 transition cursor-pointer">
